...
export function computeCompositeLayout(params: {
	...
	const webcamBaseSide = Math.min(webcamWidth, webcamHeight);
	const scale = Math.min(maxWidth / webcamBaseSide, maxHeight / webcamBaseSide);
	const side = Math.round(webcamBaseSide * scale);
	const width = side;
	const height = side;
