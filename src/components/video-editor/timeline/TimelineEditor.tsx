...
type AutoAnimationStrength = "gentle" | "balanced" | "strong";
type AutoAnimationPreset = "tutorial" | "demo" | "fast";
type OverlapHandlingMode = "trim" | "remove";

const AUTO_ANIMATION_PRESETS: Record<
	AutoAnimationPreset,
	{ durationMs: number; spacingMs: number; strength: AutoAnimationStrength }
> = {
	tutorial: { durationMs: 2200, spacingMs: 2400, strength: "gentle" },
	demo: { durationMs: 1600, spacingMs: 1800, strength: "balanced" },
	fast: { durationMs: 1000, spacingMs: 1200, strength: "strong" },
};
...
interface TimelineEditorProps {
	...
	onZoomPreviewApply?: (suggestions: { span: Span; focus: ZoomFocus }[]) => void;
	onZoomPreviewCancel?: () => void;
	...
	autoAnimateAfterRecording?: boolean;
}
...
interface ZoomPreviewCandidate {
	id: string;
	start: number;
	end: number;
	focus: ZoomFocus;
...
function Timeline({
	...
	selectedSpeedId?: string | null;
	keyframes?: { id: string; time: number }[];
	zoomPreviewCandidates?: ZoomPreviewCandidate[];
}) {
	...
	return (
		...
		>
			...
			<Row id={ZOOM_ROW_ID} isEmpty={zoomItems.length === 0} hint={t("hints.pressZoom")}>
				...
				{zoomPreviewCandidates.map((candidate) => (
					<div
						key={candidate.id}
						className="absolute top-[40px] h-[40px] rounded-md border border-dashed border-[#7be5b7]/70 bg-[#34B27B]/12 pointer-events-none"
						style={{
							left: `${valueToPixels(candidate.start - range.start)}px`,
							width: `${Math.max(8, valueToPixels(candidate.end - candidate.start))}px`,
						}}
					/>
				))}
			</Row>
...
export default function TimelineEditor({
	...
	aspectRatio,
	onAspectRatioChange,
	autoAnimateAfterRecording = false,
}: TimelineEditorProps) {
	const t = useScopedT("timeline");
	...
	const [autoAnimationStrength, setAutoAnimationStrength] =
		useState<AutoAnimationStrength>("balanced");
	const [autoAnimationDurationMs, setAutoAnimationDurationMs] = useState(1600);
	const [autoAnimationSpacingMs, setAutoAnimationSpacingMs] = useState(1800);
	const [autoAnimationPreset, setAutoAnimationPreset] = useState<AutoAnimationPreset>("demo");
	const [previewShiftStepMs, setPreviewShiftStepMs] = useState(500);
	const [overlapHandlingMode, setOverlapHandlingMode] = useState<OverlapHandlingMode>("trim");
	const [zoomPreviewCandidates, setZoomPreviewCandidates] = useState<ZoomPreviewCandidate[]>([]);
	const [autoApplyQueued, setAutoApplyQueued] = useState(false);
	...
	const handleSuggestZooms = useCallback(
		(options?: { autoApply?: boolean }) => {
			...
			if (cursorTelemetry.length < 2) {
				...
				setAutoApplyQueued(false);
				return;
			}

			const durationMultiplier =
				autoAnimationStrength === "gentle"
					? 0.85
					: autoAnimationStrength === "strong"
						? 1.2
						: 1;
			const tunedDuration = Math.round(autoAnimationDurationMs * durationMultiplier);
			const defaultDuration = Math.min(Math.max(600, tunedDuration), totalMs);
			if (defaultDuration <= 0) {
				setAutoApplyQueued(false);
				return;
			}

			const suggestionSpacingMs = Math.max(800, autoAnimationSpacingMs);
			const lockedPreviewCandidates = zoomPreviewCandidates.filter((candidate) => candidate.locked);

			const reservedSpans = [
				...zoomRegions.map((region) => ({ start: region.startMs, end: region.endMs })),
				...lockedPreviewCandidates.map((candidate) => ({ start: candidate.start, end: candidate.end })),
			...
			const acceptedCenters: number[] = lockedPreviewCandidates.map((candidate) =>
				Math.round((candidate.start + candidate.end) / 2),
			);

			const previewSuggestions: ZoomPreviewCandidate[] = [];
			...
			sortedCandidates.forEach((candidate) => {
				...
				previewSuggestions.push({
					id: `preview-${candidateStart}-${candidateEnd}-${previewSuggestions.length}`,
			...
			if (previewSuggestions.length === 0) {
				setZoomPreviewCandidates(lockedPreviewCandidates);
			...
			setZoomPreviewCandidates(
				[...lockedPreviewCandidates, ...previewSuggestions].sort((a, b) =>
					a.start === b.start ? a.end - b.end : a.start - b.start,
				),
			);
			setAutoApplyQueued(Boolean(options?.autoApply));
			toast.success(
				t("success.previewZoomSuggestionsSummary", {
					added: String(previewSuggestions.length),
					locked: String(lockedPreviewCandidates.length),
				}),
			);
		},
		[
			...
			zoomRegions,
			cursorTelemetry,
			autoAnimationStrength,
			autoAnimationDurationMs,
			autoAnimationSpacingMs,
			zoomPreviewCandidates,
			t,
		],
	);

	const handleApplyPreviewZooms = useCallback(
		(options?: { onlyUnlocked?: boolean }) => {
			if (zoomPreviewCandidates.length === 0) {
				return;
			}

			const onlyUnlocked = options?.onlyUnlocked ?? false;
			const targetCandidates = onlyUnlocked
				? zoomPreviewCandidates.filter((candidate) => !candidate.locked)
				: zoomPreviewCandidates;
			if (targetCandidates.length === 0) {
				toast.info(t("autoAnimate.noUnlockedToApply"));
				return;
			}

			const payload = [...targetCandidates]
			...
			onZoomPreviewApply?.(payload);
			setZoomPreviewCandidates((prev) =>
				onlyUnlocked ? prev.filter((candidate) => candidate.locked) : [],
			);
			setAutoApplyQueued(false);
			toast.success(
				payload.length === 1
					? t("success.appliedZoomSuggestions", { count: String(payload.length) })
					: t("success.appliedZoomSuggestionsPlural", { count: String(payload.length) }),
			);
		},
		[zoomPreviewCandidates, onZoomPreviewApply, t],
	);

	useEffect(() => {
		if (!autoApplyQueued || zoomPreviewCandidates.length === 0) {
			return;
		}
		handleApplyPreviewZooms();
	}, [autoApplyQueued, zoomPreviewCandidates, handleApplyPreviewZooms]);

	useEffect(() => {
		if (!autoAnimateAfterRecording || hasAutoAnimatedRef.current) {
			return;
		}
		if (zoomRegions.length > 0 || cursorTelemetry.length < 2 || totalMs === 0) {
			return;
		}
		hasAutoAnimatedRef.current = true;
		handleSuggestZooms({ autoApply: true });
	}, [autoAnimateAfterRecording, zoomRegions.length, cursorTelemetry.length, totalMs, handleSuggestZooms]);

	useEffect(() => {
		hasAutoAnimatedRef.current = false;
		setAutoApplyQueued(false);
	}, [totalMs]);

	const handleCancelPreviewZooms = useCallback(() => {
		if (zoomPreviewCandidates.length === 0) return;
		setZoomPreviewCandidates([]);
		onZoomPreviewCancel?.();
		toast.info(t("success.clearedZoomPreview"));
	}, [zoomPreviewCandidates.length, onZoomPreviewCancel, t]);

	const handleRemovePreviewZoom = useCallback((id: string) => {
		setZoomPreviewCandidates((prev) => prev.filter((candidate) => candidate.id !== id));
	}, []);

	const handleTogglePreviewLock = useCallback((id: string) => {
		setZoomPreviewCandidates((prev) =>
			prev.map((candidate) =>
	...
	const applyAutoAnimationPreset = useCallback((preset: AutoAnimationPreset) => {
		setAutoAnimationPreset(preset);
		const config = AUTO_ANIMATION_PRESETS[preset];
		setAutoAnimationDurationMs(config.durationMs);
		setAutoAnimationSpacingMs(config.spacingMs);
		setAutoAnimationStrength(config.strength);
	}, []);

	const handlePreviewCandidateTimeChange = useCallback(
		(id: string, field: "start" | "end", rawValue: number) => {
			if (!Number.isFinite(rawValue)) return;
			setZoomPreviewCandidates((prev) =>
				prev.map((candidate) => {
					if (candidate.id !== id || candidate.locked) return candidate;
					const value = Math.max(0, Math.min(Math.round(rawValue), totalMs));
					const minDuration = 150;
					if (field === "start") {
						const nextStart = Math.min(value, candidate.end - minDuration);
						return { ...candidate, start: Math.max(0, nextStart) };
					}
					const nextEnd = Math.max(value, candidate.start + minDuration);
					return { ...candidate, end: Math.min(totalMs, nextEnd) };
				}),
			);
		},
		[totalMs],
	...
	const handleSortPreviewZooms = useCallback(() => {
		setZoomPreviewCandidates((prev) =>
			[...prev].sort((a, b) => (a.start === b.start ? a.end - b.end : a.start - b.start)),
		);
	}, []);

	const handleResolvePreviewOverlaps = useCallback(() => {
		setZoomPreviewCandidates((prev) => {
			const sorted = [...prev].sort((a, b) => (a.start === b.start ? a.end - b.end : a.start - b.start));
			if (overlapHandlingMode === "remove") {
				const compacted: ZoomPreviewCandidate[] = [];
				sorted.forEach((candidate) => {
					const last = compacted[compacted.length - 1];
					if (!last || candidate.start >= last.end || candidate.locked) {
			...
			const minDuration = 150;
			const trimmed: ZoomPreviewCandidate[] = [];
			sorted.forEach((candidate) => {
				...
				if (candidate.start >= last.end || candidate.locked) {
					trimmed.push(candidate);
					return;
				}
				const adjustedStart = Math.min(candidate.end - minDuration, Math.max(last.end, candidate.start));
				if (candidate.end - adjustedStart >= minDuration) {
					trimmed.push({ ...candidate, start: adjustedStart });
				}
			});
			return trimmed;
		});
		if (zoomPreviewCandidates.some((candidate) => candidate.locked)) {
			toast.info(t("autoAnimate.lockedSkippedHint"));
		}
	}, [overlapHandlingMode, zoomPreviewCandidates, t]);

	const handleShiftPreviewZooms = useCallback(
		(deltaMs: number) => {
			...
			setZoomPreviewCandidates((prev) =>
				prev.map((candidate) => {
					...
					const nextStart = Math.max(0, Math.min(totalMs - duration, candidate.start + deltaMs));
					return {
						...candidate,
						start: nextStart,
						end: Math.min(totalMs, nextStart + duration),
					};
				}),
			);
		},
	...
	return (
		<div className="flex-1 flex flex-col bg-[linear-gradient(180deg,#0d0f13_0%,#0a0b0e_100%)] overflow-hidden">
			<div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(52,178,123,0.12),transparent_40%),linear-gradient(180deg,#13161d_0%,#0f1116_100%)]">
					...
					<Popover>
						<PopoverTrigger asChild>
							<Button
								variant="ghost"
								size="sm"
								className="h-7 px-2 text-[11px] text-[#34B27B] bg-[#34B27B]/10 border border-[#34B27B]/25 hover:bg-[#34B27B]/15 transition-all"
								title={t("buttons.autoAnimate")}
							>
								{t("buttons.autoAnimate")}
							</Button>
						</PopoverTrigger>
						<PopoverContent
							align="start"
							sideOffset={8}
							className="w-[300px] border-white/10 bg-[#131720] text-slate-100 p-3 rounded-xl"
						>
							<div className="space-y-3">
								<div>
									<p className="text-xs font-semibold text-slate-100">{t("autoAnimate.title")}</p>
									<p className="text-[10px] text-slate-400 mt-1">{t("autoAnimate.description")}</p>
								</div>

								<div className="space-y-1.5">
									<p className="text-[10px] text-slate-300">{t("autoAnimate.presets.title")}</p>
									<div className="grid grid-cols-3 gap-1.5">
										{(["tutorial", "demo", "fast"] as const).map((preset) => (
											<button
												key={preset}
												type="button"
												onClick={() => applyAutoAnimationPreset(preset)}
												className={cn(
													"h-7 rounded-md text-[10px] border transition-all",
													autoAnimationPreset === preset
														? "bg-[#34B27B]/15 border-[#34B27B]/50 text-[#7be5b7]"
														: "bg-black/20 border-white/10 text-slate-400 hover:text-slate-200",
												)}
											>
												{t(`autoAnimate.presets.${preset}`)}
											</button>
										))}
									</div>
								</div>
								...
								<div className="space-y-1.5">
									<div className="flex items-center justify-between text-[10px] text-slate-300">
										<span>{t("autoAnimate.duration")}</span>
										<span className="text-[#34B27B] font-medium">{autoAnimationDurationMs}ms</span>
									</div>
									<Slider
										value={[autoAnimationDurationMs]}
										onValueChange={(values) => setAutoAnimationDurationMs(Math.round(values[0]))}
										min={600}
										max={3500}
										step={100}
								...
								<div className="space-y-1.5">
									<div className="flex items-center justify-between text-[10px] text-slate-300">
										<span>{t("autoAnimate.spacing")}</span>
										<span className="text-[#34B27B] font-medium">{autoAnimationSpacingMs}ms</span>
									</div>
									<Slider
										value={[autoAnimationSpacingMs]}
										onValueChange={(values) => setAutoAnimationSpacingMs(Math.round(values[0]))}
										min={800}
										max={5000}
										step={100}
										className="w-full [&_[role=slider]]:bg-[#34B27B] [&_[role=slider]]:border-[#34B27B] [&_[role=slider]]:h-3 [&_[role=slider]]:w-3"
								...
								<div className="grid grid-cols-3 gap-1.5">
									{(["gentle", "balanced", "strong"] as const).map((level) => (
										<button
											key={level}
											type="button"
											onClick={() => setAutoAnimationStrength(level)}
											className={cn(
												"h-7 rounded-md text-[10px] border transition-all",
												autoAnimationStrength === level
													? "bg-[#34B27B]/15 border-[#34B27B]/50 text-[#7be5b7]"
													: "bg-black/20 border-white/10 text-slate-400 hover:text-slate-200",
											)}
										>
											{t(`autoAnimate.strength.${level}`)}
										</button>
									))}
								</div>

								{zoomPreviewCandidates.length > 0 && (
									<div className="space-y-2 rounded-lg border border-[#34B27B]/30 bg-[#34B27B]/10 p-2">
										<p className="text-[10px] text-[#9df0ca] font-medium">
											{t("autoAnimate.previewCount", {
												count: String(zoomPreviewCandidates.length),
											})}
										</p>
										<div className="grid grid-cols-2 gap-1">
											<Button
												type="button"
												variant="outline"
												onClick={handleSortPreviewZooms}
												className="h-6 text-[10px] border-white/20 bg-black/20 hover:bg-black/35"
											>
												{t("autoAnimate.sortByTime")}
											</Button>
											<Button
												type="button"
												variant="outline"
												onClick={handleResolvePreviewOverlaps}
												className="h-6 text-[10px] border-white/20 bg-black/20 hover:bg-black/35"
											>
												{t("autoAnimate.resolveOverlaps")}
											</Button>
										</div>
										<div className="grid grid-cols-2 gap-1">
											<button
												type="button"
												onClick={() => setOverlapHandlingMode("trim")}
												className={cn(
													"h-6 rounded-md text-[10px] border transition-all",
													overlapHandlingMode === "trim"
														? "bg-[#34B27B]/15 border-[#34B27B]/50 text-[#7be5b7]"
														: "bg-black/20 border-white/10 text-slate-400 hover:text-slate-200",
												)}
											>
												{t("autoAnimate.overlapTrim")}
											</button>
											<button
												type="button"
												onClick={() => setOverlapHandlingMode("remove")}
												className={cn(
													"h-6 rounded-md text-[10px] border transition-all",
													overlapHandlingMode === "remove"
														? "bg-[#34B27B]/15 border-[#34B27B]/50 text-[#7be5b7]"
														: "bg-black/20 border-white/10 text-slate-400 hover:text-slate-200",
												)}
											>
												{t("autoAnimate.overlapRemove")}
											</button>
										</div>
										<div className="space-y-1">
											<p className="text-[10px] text-slate-300">{t("autoAnimate.shiftStep")}</p>
											<div className="grid grid-cols-4 gap-1">
												{([100, 250, 500, 1000] as const).map((step) => (
													<button
														key={step}
														type="button"
														onClick={() => setPreviewShiftStepMs(step)}
														className={cn(
															"h-6 rounded-md text-[10px] border transition-all",
															previewShiftStepMs === step
																? "bg-[#34B27B]/15 border-[#34B27B]/50 text-[#7be5b7]"
																: "bg-black/20 border-white/10 text-slate-400 hover:text-slate-200",
														)}
													>
										...
										<div className="grid grid-cols-2 gap-1">
											<Button
												type="button"
												variant="outline"
												onClick={() => handleShiftPreviewZooms(-previewShiftStepMs)}
												className="h-6 text-[10px] border-white/20 bg-black/20 hover:bg-black/35"
											>
												{t("autoAnimate.shiftLeft", { step: String(previewShiftStepMs) })}
											</Button>
											<Button
												type="button"
												variant="outline"
												...
												className="h-6 text-[10px] border-white/20 bg-black/20 hover:bg-black/35"
											>
												{t("autoAnimate.shiftRight", { step: String(previewShiftStepMs) })}
											</Button>
										</div>
										<div className="max-h-20 overflow-y-auto custom-scrollbar space-y-1">
											{zoomPreviewCandidates.map((candidate) => (
												<div
													key={candidate.id}
													className={cn(
														"space-y-1 text-[10px] text-slate-200/90 border rounded px-1.5 py-1",
														candidate.locked
															? "bg-[#34B27B]/15 border-[#34B27B]/35"
															: "bg-black/20 border-white/10",
													)}
												>
													<div className="flex items-center gap-1">
														<span className="flex-1">
															{formatTimeLabel(candidate.start, 100)} - {formatTimeLabel(candidate.end, 100)}
														</span>
														<button
															type="button"
															onClick={() => handleTogglePreviewLock(candidate.id)}
															className="w-4 h-4 rounded hover:bg-white/10 flex items-center justify-center text-slate-400 hover:text-white"
															title={candidate.locked ? t("autoAnimate.unlockPreview") : t("autoAnimate.lockPreview")}
														>
															{candidate.locked ? <Lock className="w-3 h-3" /> : <LockOpen className="w-3 h-3" />}
														</button>
														<button
															type="button"
															onClick={() => handleRemovePreviewZoom(candidate.id)}
															className="w-4 h-4 rounded hover:bg-white/10 flex items-center justify-center text-slate-400 hover:text-white"
															title={t("autoAnimate.removePreview")}
														>
															<X className="w-3 h-3" />
														</button>
													</div>
													<div className="grid grid-cols-2 gap-1">
														<input
															key={`${candidate.id}-start-${candidate.start}`}
															type="number"
															min={0}
															max={totalMs}
															defaultValue={candidate.start}
															disabled={candidate.locked}
															onBlur={(e) =>
																handlePreviewCandidateTimeChange(
																	candidate.id,
																	"start",
																	Number(e.target.value),
																)
															}
															className="h-6 rounded border border-white/10 bg-black/30 px-1 text-[10px] disabled:opacity-50"
														/>
														<input
															key={`${candidate.id}-end-${candidate.end}`}
															type="number"
															min={0}
															max={totalMs}
															defaultValue={candidate.end}
															disabled={candidate.locked}
															onBlur={(e) =>
																handlePreviewCandidateTimeChange(
																	candidate.id,
										...
										<div className="grid grid-cols-3 gap-1.5">
											<Button
												type="button"
												variant="outline"
												onClick={handleCancelPreviewZooms}
												className="h-7 text-[10px] border-white/20 bg-black/20 hover:bg-black/35"
											>
												{t("autoAnimate.cancelPreview")}
											</Button>
											<Button
												type="button"
												variant="outline"
												onClick={() => handleApplyPreviewZooms({ onlyUnlocked: true })}
												className="h-7 text-[10px] border-[#34B27B]/35 bg-[#34B27B]/10 hover:bg-[#34B27B]/20 text-[#9df0ca]"
											>
												{t("autoAnimate.applyUnlocked")}
											</Button>
											<Button
												type="button"
												onClick={() => handleApplyPreviewZooms()}
												className="h-7 text-[10px] bg-[#34B27B] hover:bg-[#34B27B]/90 text-white"
											>
												{t("autoAnimate.applyPreview")}
											</Button>
										</div>
									</div>
								)}
								...
								<Button
									onClick={() => handleSuggestZooms()}
									className="w-full h-8 text-xs bg-[#34B27B] hover:bg-[#34B27B]/90 text-white"
								>
									{t("buttons.autoAnimateGenerate")}
								</Button>
							</div>
						</PopoverContent>
					</Popover>
			...
			>
				...
				>
					...
					<Timeline
						...
						keyframes={keyframes}
						zoomPreviewCandidates={zoomPreviewCandidates}
					/>
				</TimelineWrapper>
			</div>
		</div>
