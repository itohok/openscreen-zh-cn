...
export function SettingsPanel({
	...
	const copyDonateText = async (text: string, successKey: string) => {
		try {
			const copiedViaElectron = await window.electronAPI?.copyToClipboard(text);
			if (copiedViaElectron?.success) {
			...
			toast.error(t("donate.copyFailed"));
		}
	};

	const copyFeedbackDeviceInfo = async () => {
		...
		await copyDonateText(diagnostics, "feedback.copyDeviceInfoSuccess");
	};
	...
	return (
		<div className="flex-[2] min-w-0 rounded-2xl flex flex-col h-full overflow-hidden border border-white/10 bg-[radial-gradient(circle_at_top,rgba(52,178,123,0.08),transparent_38%),linear-gradient(180deg,#0f1115_0%,#090a0d_100%)] shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
			<div className="flex-1 overflow-y-auto custom-scrollbar p-4 pb-0">
				...
				>
					{hasWebcam && (
						<AccordionItem
							value="layout"
							className="border-white/10 rounded-xl bg-[linear-gradient(180deg,rgba(255,255,255,0.05)_0%,rgba(255,255,255,0.02)_100%)] px-3 backdrop-blur-sm"
						>
							...
							<AccordionContent className="pb-3">
								<div className="p-2.5 rounded-lg bg-[linear-gradient(180deg,rgba(255,255,255,0.06)_0%,rgba(255,255,255,0.03)_100%)] border border-white/10">
					...
					<AccordionItem value="effects" className="border-white/10 rounded-xl bg-[linear-gradient(180deg,rgba(255,255,255,0.05)_0%,rgba(255,255,255,0.02)_100%)] px-3 backdrop-blur-sm">
						<AccordionTrigger className="py-2.5 hover:no-underline">
							<div className="flex items-center gap-2">
								<Sparkles className="w-4 h-4 text-[#34B27B]" />
						...
						<AccordionContent className="pb-3">
							<div className="grid grid-cols-2 gap-2 mb-3">
								<div className="flex items-center justify-between p-2.5 rounded-lg bg-[linear-gradient(180deg,rgba(255,255,255,0.06)_0%,rgba(255,255,255,0.03)_100%)] border border-white/10">
							...
							<div className="grid grid-cols-2 gap-2">
								...
								<div
									className={`p-2.5 rounded-lg bg-[linear-gradient(180deg,rgba(255,255,255,0.06)_0%,rgba(255,255,255,0.03)_100%)] border border-white/10 ${webcamLayoutPreset === "vertical-stack" ? "opacity-40 pointer-events-none" : ""}`}
								>
									<div className="flex items-center justify-between mb-1">
										<div className="text-[10px] font-medium text-slate-300">
											{t("effects.padding")}
					...
					>
						...
						<AccordionContent className="pb-3">
							<Tabs defaultValue="image" className="w-full">
								<TabsList className="mb-2 bg-black/30 border border-white/10 p-0.5 w-full grid grid-cols-3 h-8 rounded-lg shadow-inner">
									<TabsTrigger
										value="image"
								...
								<div className="max-h-[min(200px,25vh)] overflow-y-auto custom-scrollbar">
									<TabsContent value="image" className="mt-0 space-y-2">
										...
										<Button
											...
											variant="outline"
											className="w-full gap-2 bg-black/25 text-slate-200 border-white/15 hover:bg-[#34B27B] hover:text-white hover:border-[#34B27B] transition-all h-7 text-[10px]"
										>
			...
			<div className="flex-shrink-0 p-4 pt-3 border-t border-white/10 bg-[linear-gradient(180deg,#11141a_0%,#0b0d12_100%)]">
				...
				<div className="grid grid-cols-2 gap-2 mt-3 border-t border-white/5 pt-2">
					<button
						type="button"
						onClick={() => setFeedbackDialogOpen(true)}
						className="flex items-center justify-center gap-1.5 text-[10px] text-slate-500 hover:text-slate-300 py-1.5 rounded-md hover:bg-white/5 transition-colors"
					>
					...
					<button
						...
						className="flex items-center justify-center gap-1.5 text-[10px] text-slate-500 hover:text-slate-200 py-1.5 rounded-md hover:bg-[#34B27B]/10 transition-colors"
					>
						<Heart className="w-3 h-3 text-[#34B27B]" />
						{t("links.donate")}
					</button>
				</div>

				<Dialog open={feedbackDialogOpen} onOpenChange={setFeedbackDialogOpen}>
					<DialogContent className="bg-[#101114] border border-white/10 text-slate-100 max-w-lg rounded-2xl p-0 overflow-hidden">
						<div className="bg-[radial-gradient(circle_at_top_right,rgba(52,178,123,.18),transparent_45%),linear-gradient(180deg,#121418_0%,#0b0d10_100%)] p-6">
							<DialogHeader>
								<DialogTitle className="text-xl font-semibold tracking-tight">{t("feedback.title")}</DialogTitle>
								<DialogDescription className="text-slate-400 mt-1">{t("feedback.description")}</DialogDescription>
							</DialogHeader>

							<p className="mt-4 text-sm text-slate-300">{t("feedback.simpleHint")}</p>
							<div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
								<Button
									type="button"
									variant="outline"
									onClick={copyFeedbackDeviceInfo}
									className="h-9 text-xs border-white/15 bg-white/[0.02] hover:bg-white/[0.08]"
								>
									{t("feedback.copyDeviceInfo")}
								</Button>
								<Button
									type="button"
									variant="outline"
									onClick={() => copyDonateText("bazhaoxiong", "feedback.copyWechatSuccess")}
									className="h-9 text-xs border-white/15 bg-white/[0.02] hover:bg-white/[0.08]"
								>
				...
				<Dialog open={donateDialogOpen} onOpenChange={setDonateDialogOpen}>
					<DialogContent className="bg-[#101114] border border-white/10 text-slate-100 max-w-2xl rounded-2xl p-0 overflow-hidden">
						<div className="bg-[radial-gradient(circle_at_top_right,rgba(52,178,123,.18),transparent_45%),linear-gradient(180deg,#121418_0%,#0b0d10_100%)] p-6">
							<DialogHeader>
								<DialogTitle className="text-xl font-semibold tracking-tight">{t("donate.title")}</DialogTitle>
								<DialogDescription className="text-slate-300 mt-1">{t("donate.descriptionFunny")}</DialogDescription>
							</DialogHeader>

							<div className="mt-3 rounded-xl border border-amber-300/30 bg-amber-300/10 px-3 py-2 text-xs text-amber-100 leading-relaxed">
								{t("donate.freeNotice")}
							</div>

							<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
								<div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
									<p className="text-xs text-slate-300 mb-3">{t("donate.rewardCode")}</p>
									<div className="rounded-lg overflow-hidden border border-white/10 bg-black/20">
										<img src={rewardCodePath} alt={t("donate.rewardCodeAlt")} className="w-full h-auto" />
									</div>
								</div>
								<div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
									<p className="text-xs text-slate-300 mb-3">{t("donate.officialQr")}</p>
									<div className="rounded-lg overflow-hidden border border-white/10 bg-black/20">
										<img src={officialQrPath} alt={t("donate.officialQrAlt")} className="w-full h-auto" />
									</div>
								</div>
							</div>

							<p className="mt-4 text-sm text-slate-300">
								{t("donate.followPrefix")} <span className="text-[#34B27B] font-semibold">微信号：bazhaoxiong</span>
							</p>
							<div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
								<Button
									type="button"
									variant="outline"
									onClick={() => copyDonateText("bazhaoxiong", "donate.copyOfficialNameSuccess")}
									className="h-8 text-xs border-white/15 bg-white/[0.02] hover:bg-white/[0.08]"
								>
									{t("donate.copyOfficialName")}
								</Button>
								<Button
									type="button"
									variant="outline"
									onClick={() => copyDonateText("https://nieyinie.com/", "donate.copyProjectLinkSuccess")}
									className="h-8 text-xs border-white/15 bg-white/[0.02] hover:bg-white/[0.08]"
								>
									{t("donate.copyProjectLink")}
								</Button>
							</div>
						</div>
					</DialogContent>
